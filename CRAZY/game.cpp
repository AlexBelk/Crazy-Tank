	...
	for( int r = 0; r < levelRows; r++ )
	{
		for( int c = 0; c < levelColumns; c++ )
		{
			...
			switch( cellSymbol )
			{
				...
				{
					GameObject* wall = createObject(GameObjectType_Wall,
						(c + 0.5),
						r,
						GetRenderCellSymbol(cellSymbol),
						GetRenderCellSymbolColor(cellSymbol),
...
void Game::render()
{
	...
	// Draw objects count
	char buff[64];
	sprintf_s(buff, "LIFE: %d Enemies Down: %d",  Lifecount, TargetDown);
					 ...
					 else
					 {
						 for (int e = 0; e < gameObjectsCountMax; e++)
						 {
							 ...
							 if (anotherObject != 0)
							 {
								 ...
								 if (anotherObject->getType() == GameObjectType_Wall)
								 {
									 if (anotherObject->intersects(object))
									 {
										 destroyObject(object);
